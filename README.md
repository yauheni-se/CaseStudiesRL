...
The results indicated that in case of 4x4 game grid, the probability of success for MC Off policy was on average 5 percentage points higher. The down direction at the very beginning of the game (S=1, '???') was the bottleneck for the On policy algorithm. This direction more often was leading to situations where the algorithm was endlessly hitting the upper left corner, where there was a greater chance of slipping and hitting the ice hole. A good choice for the first action plays a big role in this game.
...
# GridWorld
Case study concentrates on implementing SARSA(??) algorithm and further comparison with SARSA(0) for the GridWorld game.

Based on the results, SARSA(??=0.5) turned out to be the most effective for the considered GridWorld problem. This is evidenced by the agent's total reward (score), since for ??=0.5 the number of completed games (episodes) was the largest (729), so as the average episodes before completion (=160). It is worth noting, however, that the obtained results may have been obtained by chance, as the average difference (371-325)/325 = 14.15% is not too large and may result from the random nature of learning.